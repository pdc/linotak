...
class Property(StuffBase):
    ...
    def __str__(self):
        return "%s=%r" % (
            self.classes[0] if len(self.classes == 1) else self.classes,
            self.value,
        )
...
class LinkRecognizer:
    ...
    def handle_start_link(self, tag):
        ...
        if rel and href is not None:
            tag.is_link = Link(
                ...
                normalize_whitespace(tag.get("type")),
                normalize_whitespace(tag.get("title")),
            )
    ...
    def handle_end_input(self, tag):
        if any(x.startswith("u-") for x in tag.classes):
            ...
            return [
                Link(
                    None, urljoin(self.base_url, tag.get("value")), classes=tag.classes
                )
            ]
...
class PropertyRecognizer:
    ...
    def handle_start(self, tag):
        prop_names = [
            x for x in tag.classes if x.startswith("p-") or x.startswith("dt-")
        ]
        if prop_names:
...
class OGRecognizer:
    ...
    def handle_end_meta(self, tag):
        if (
            ...
            and name.startswith("og:")
            or (name := tag.get("name"))
    ...
    def handle_end_body(self, tag):
        image_src = (
            src := self.props.pop("og:image", None)
            or self.props.pop("twitter:image", None)
        ) and urljoin(self.base_url, src)
        title = self.props.pop("og:title", None) or self.props.pop(
            "twitter:title", None
        )
        desc = self.props.pop("og:description", None) or self.props.pop(
        ...
        if title or desc or url:
            ...
            result.append(
                HEntry(url, title, desc, images=image_src and [Img(image_src)])
            )
        elif image_src:
...
class HSomethingRecognizer:
    def handle_stuff(self, tag, stuff):
        ...
        if h_classes:
            ...
            return [
                self.make_something(tag, h, other_classes, stuff) for h in h_classes
            ]
    ...
    def make_h_cite(self, tag, classes, stuff):
        link = _pop_stuff_strictly(stuff, Link, "h-cite") or _pop_stuff(
            stuff, Link, "u-url"
        )
...
class PageScanner(HTMLParser):
    ...
    def __init__(self, base_url, recognizers=None, *args, **kwargs):
        ...
        self.recognizers = [
            ...
            for x in (recognizers or PageScanner.default_recognizers)
        ]
        self.base_url = base_url
        if base_url:
    ...
    def _pop_tag(self):
        ...
        stuffs.extend(
            self.iter_notify("handle_end_%s" % tag.name.replace("-", "_"), tag)
        )

...
class SubdomainSeriesMiddleware:
    ...
    def __init__(self, get_response):
        ...
        if not hasattr(settings, 'NOTES_DOMAIN'):
            raise MiddlewareNotUsed('NOTES_DOMAIN not set so subdomain middleware not used')
        domain = settings.NOTES_DOMAIN
        if not domain:
            raise MiddlewareNotUsed('NOTES_DOMAIN empty so subdomain middleware not used')

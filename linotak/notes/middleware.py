...
class SubdomainSeriesMiddleware:
    ...
    def __init__(self, get_response):
        ...
        if not hasattr(settings, 'NOTES_DOMAIN'):
            raise MiddlewareNotUsed('NOTES_DOMAIN not set so subdomain middlware not used')
        domain = settings.NOTES_DOMAIN
        if not domain:
            raise MiddlewareNotUsed('NOTES_DOMAIN empty so subdomain middlware not used')

        self.subdomain_re = re.compile(r'^([a-z0-9]{1,63})\.' + re.escape(domain) + '$')

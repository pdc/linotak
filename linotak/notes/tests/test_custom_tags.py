...
class TestNoteListUrl(TestCase):
    def test_can_link_to_fully_specified_list(self):
        ...
        self.assertEqual(
            result, "https://zerg.example.org/tagged/bar+foo/drafts/page13/"
        )
    ...
    def xtest_uses_a_star_for_all_series(self):
        result = note_list_url(
            {}, series=None, tag_filter=TagFilter(["wat"]), drafts=False, page=69
        )
...
class TestNoteUrl(TestCase):
    ...
    def test_can_pass_view_name(self):
        ...
        self.assertEqual(
            result, "https://zerg.example.org/tagged/bar+foo/drafts/69/edit"
        )

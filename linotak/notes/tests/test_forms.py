...
class TestNoteForm(TestCase):
    ...
    def test_new_form_acquires_author_from_series(self):
        ...
        self.assertEqual(set(form.fields['author'].queryset.all()), {self.alice, self.bob})

    def test_acquires_initial_values_from_instance(self):
        note = NoteFactory.create(series=self.series, text='Foo', author=self.alice, subjects=[

...
class TestNoteTitle(TestCase):
    ...
    def test_shortens_long_single_paragraph(self):
        result = Note(
            text="This is a very long paragraph contrived to illustrate the shortening of note titles"
        ).short_title()
    ...
    def test_takes_first_line_if_multiline(self):
        result = Note(
            text="Short title\nThis is a very long paragraph contrived to illustrate the shortening of note titles"
        ).short_title()
    ...
    def test_choosese_words_to_make_30_chars_or_so(self):
        result = Note(
            text="A web app for designing computer keyboard layouts!"
        ).short_title()
...
class TestNoteExtractSubjects(TestCase):
    ...
    def test_matches_multiple_urls(self):
        note = NoteFactory.create(
            text="Many! http://example.com/1 https://example.com/2"
        )
        ...
        self.assertEqual(
            {x.url for x in note.subjects.all()},
            {"http://example.com/1", "https://example.com/2"},
        )
    ...
    def test_matches_via_between_URLs(self):
        note = NoteFactory.create(
            text="https://example.com/1 via https://example.com/2"
        )
        ...
        self.assertEqual(
            [x.url for x in note.subjects.all()], ["https://example.com/1"]
        )
    ...
    def test_extracts_hashtags(self):
        note = NoteFactory.create(
            text="HelloWorld! #greeting https://example.com/1/ #camelCase"
        )
    ...
    def test_extracts_hashtags_at_start(self):
        ...
        self.assertEqual(
            {x.name for x in note.tags.all()}, {"tokyocameraclub", "japan", "torii"}
        )

    def test_deletes_unwanted_tags(self):
        note = NoteFactory.create(
            tags=["bimble", "unwanted"], text="Lol #wimble #bimble"
        )
    ...
    def test_sets_nsfw_flag_on_locator_with_via(self):
        note = NoteFactory(
            text="Yo https://example.com/1 (nsfw) via https://example.com/2"
        )
...
class TestNoteTextWithLinks(TestCase):
    ...
    def test_makes_tags_camel_case(self):
        ...
        self.assertEqual(note.text_with_links(), "Hello, world\n\n#bambooFirefly")
    ...
    def test_adds_nsfw(self):
        ...
        self.assertEqual(
            note.text_with_links(), "Hello, world\n\nhttps://example.com/1 (nsfw)"
        )
    ...
    def test_optionally_adds_permashortcitation(self):
        ...
        self.assertEqual(
            result,
            f"Hello, world!! (seriesname.example.net {note.pk})\n\nhttps://example.com/1",
        )
    ...
    def test_shortens_text_and_substitutes_link_to_self_of_overlength(self):
        note = NoteFactory.create(
            ...
            subjects=[
                LocatorFactory.create(
                    url="https://example.com/2020/10/25/2020-10-25-link-of-more-than-23-characters/"
                )
            ],
    ...
    def test_includes_via_links_in_calculation_omits_ellipsis_if_text_not_shorter(self):
        ...
        self.assertEqual(result, expected)
        self.assertLess(
            len(result.replace(f"https://seriesname.example.net/{note.pk}", "*" * 23)),
            100,
        )  # Check this is actually the correct result.
...
class TestEffectiveCharCount(TestCase):
    def test_includes_via_urls(self):
        self.assertEqual(
            effective_char_count(
                "Hello", [], [LocatorFactory(via=LocatorFactory())], url_length=23
            ),
            5 + 2 + 23 + 6 + 23,
        )
    ...
    def test_includes_hashtags(self):
        self.assertEqual(
            effective_char_count(
                "Hello", [TagFactory(name="someTag")], [LocatorFactory()], url_length=23
            ),
            5 + 2 + 8 + 2 + 23,
        )
...
class TestNoteAbsoluteUrl(TestCase):
    ...
    def test_allows_replacing_view(self):
        ...
        self.assertEqual(
            note.get_absolute_url(view="edit"), "/drafts/%d/edit" % note.pk
        )
    ...
    def test_can_add_tags(self):
        ...
        self.assertEqual(
            note.get_absolute_url(tag_filter=TagFilter.parse("foo+bar")),
            "/tagged/bar+foo/drafts/%d" % note.pk,
        )
    ...
    def test_can_make_include_host(self):
        ...
        with self.settings(NOTES_DOMAIN="notes.example.org"):
            self.assertEqual(
                note.get_absolute_url(with_host=True),
                "https://bomp.notes.example.org/%d" % note.pk,
            )
    ...
    def test_can_suppress_HTTPS_for_test_server(self):
        ...
        with self.settings(
            NOTES_DOMAIN="notes.example.org", NOTES_DOMAIN_INSECURE=True
        ):
            self.assertEqual(
                note.get_absolute_url(with_host=True),
                "http://bomp.notes.example.org/%d" % note.pk,
            )
    ...
    def test_queues_fetch_when_locator_created(self):
        ...
        with self.settings(NOTES_FETCH_LOCATORS=True), patch.object(
            ...
            fetch_locator_page.delay.assert_called_once_with(
                locator.pk, if_not_scanned_since=None
            )
...
class TestLocatorQueueFetch(TransactionTestCase):
    ...
    def test_value_that_can_return_original_datetime(self):
        ...
        fetch_locator_page.delay.assert_called_once_with(
            locator.pk, if_not_scanned_since=DateTimeTimestampMatcher(locator.scanned)
        )
            ...
            [
                LocatorImage(
                    ...
                    image=Image.objects.create(
                        data_url="https://example.com/1", width=500, height=300
                    ),
                ),
                LocatorImage(
                    locator=locator,
                    image=Image.objects.create(
                        data_url="https://example.com/2", width=500, height=400
                    ),
                    prominence=3,
                ),
                LocatorImage(
                    locator=locator,
                    image=Image.objects.create(
                        data_url="https://example.com/3", width=500, height=500
                    ),
                ),
            ]
        )
        ...
        LocatorImage.objects.bulk_create(
            [
                LocatorImage(
                    locator=locator,
                    image=Image.objects.create(
                        data_url="https://example.com/100", width=100, height=50
                    ),
                ),
                LocatorImage(
                    locator=locator,
                    image=Image.objects.create(
                        data_url="https://example.com/500", width=500, height=400
                    ),
                ),
                LocatorImage(
                    locator=locator,
                    image=Image.objects.create(
                        data_url="https://example.com/50", width=60, height=60
                    ),
                ),
            ]
        )

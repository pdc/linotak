...
class ConnectionCreateView(LoginRequiredMixin, SeriesMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        connection = Connection.objects.create_connection(
            form.cleaned_data["series"], form.cleaned_data["domain"]
        )
...
def callback(request):
    ...
    authorization_response = connection.series.make_absolute_url(
    ...
    r = oauth.get(
        connection.verify_credentials_url, headers={"Accept": "application/json"}
    )
    ...
    return render(
        request, "mastodon/connection_failed.html", {"response": r, **r.json()}
    )
